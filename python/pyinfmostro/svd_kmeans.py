...
def plot_results_ars(xsection_name, datacarril_list, workspace, x, s, vh, means, labels, clabels, fv):
    ars = int(len(s)/2)
    ...
    ap1.set_xticks(datacarril_list)
    ap1.grid()
    ...
    ap2.set_xticks(datacarril_list)
    ap2.grid()
...
def plot_results_flexi(xsection_name, datacarril_list, workspace, x, s, vh, means, labels, clabels, fv):
    cumfv = np.sum(fv, axis=1)
    zippedfv = list(zip(cumfv, clabels))
    zippedorder = np.array(sorted(zippedfv, key = lambda x: x[0]), dtype=int)
    order = zippedorder[:,1].tolist()
    labelnames = ["C0", "C1", "C2", "C3", "C4", "C5", "C6", "C7"]
    ...
    np.savetxt(os.path.join(workspace, "means.csv"), means, delimiter=",")
    np.savetxt(os.path.join(workspace, "singular.csv"), means, delimiter=",")
    ...
    for i in clabels:
        #print(fv[i,0:ars])
        ap1.plot(fv[i,:],'-o', c=means_lc[j], linewidth=2)
        j+=1
    ap1.set_xlabel('ars lane')
    ap1.set_ylabel('Vehicular flow\n[ Veh. class1 / 5 mins ]')
    ap1.set_xticklabels(datacarril_list)
    ap1.grid()
    j = 0
    fig.legend([ap1], labels=labelorder, loc="center right", borderaxespad=0.5, title="Cluster ID")
    fig.savefig(os.path.join(workspace,"profiles.png"))
    plt.close()
